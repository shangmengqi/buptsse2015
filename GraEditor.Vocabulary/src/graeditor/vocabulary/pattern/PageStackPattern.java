...
public class PageStackPattern extends IdPattern implements IPattern{
	
	private static IColorConstant FONT_COLOR = ColorConstant.BLACK;
	
	public PageStackPattern() {
	...
	public Object[] create(ICreateContext context) {
		...
		String ret = ExampleUtil.askString("Text", "Please input a text", "PageStack " + i);	
		try {
			value = ret;
		} catch (Exception e) {
		}
		
		PageStackModule pageStackModule = Vocabulary_modelFactory.eINSTANCE.createPageStackModule();			
		pageStackModule.setName("PageStack");
		pageStackModule.setTextContent(value);
		getDiagram().eResource().getContents().add(pageStackModule);
		addGraphicalRepresentation(context, pageStackModule);
		return new Object[] { pageStackModule };
	}
	...
	protected PictogramElement doAdd(IAddContext context) {
		PageStackModule module = (PageStackModule) context.getNewObject();
		Diagram diagram = (Diagram) context.getTargetContainer();
		
		IPeCreateService peCreateService = Graphiti.getPeCreateService();
		ContainerShape containerShape = peCreateService.createContainerShape(diagram, true);
		
		GraPropertyUtil.setValue(containerShape, "shape_id", "PageStack"+IDGenerator.generateID());
		...
		Image file = gaService.createImage(containerShape, VocabularyImageProvider.PAGESTACK);
		gaService.setLocationAndSize(file, context.getX(), context.getY(), width, height);
				
		String text = null;
		
		text = "" + ((PageStackModule)module).getTextContent();
	...
	public boolean isMainBusinessObjectApplicable(Object mainBusinessObject) {
		// TODO Auto-generated method stub
		return mainBusinessObject instanceof PageStackModule;
	}
