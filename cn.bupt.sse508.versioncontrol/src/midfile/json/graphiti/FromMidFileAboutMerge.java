...
public class FromMidFileAboutMerge {
	...
	// 所有节点的数组
		 ...
		 * @return
		 */
		public static String fromMidFileAboutMerge(String content, JsonObject merJsonObject) {
			// 对合并文件行处理，便于在对节点的扫描中一并处理
			System.out.println("\n\n-----------------------------");
			System.out.println("开始将json格式转为xml格式");
			Map<String, JsonObject> mergeNodeMap = new HashMap<String, JsonObject>();
			if (null != merJsonObject) { // 存在合并
				JsonArray mergeNodeArray = merJsonObject.get("merge_node").getAsJsonArray();
				if (mergeNodeArray.size() > 0) { // 存在节点的合并情况
					for (int i = 0; i < mergeNodeArray.size(); i++) {
						System.out.println(mergeNodeArray.get(i).toString());
						String shap_id = mergeNodeArray.get(i).getAsJsonObject().get("shap_id").getAsString();
						mergeNodeMap.put(shap_id, mergeNodeArray.get(i).getAsJsonObject());
					}					
				}				
			}
			...
			// 处理空的link数组的情况
			{
				...
				if (arr.length() == 0) {
					...
					if (foo.getJSONObject("businessObjects").getString("@href").equals("")) { // 空的href，需要删掉link
						...
						// 不为空的情况，将其转换为xml文件 
						System.out.println("跳转不为空的情况暂时未处理");
					}
				}
			}
...
//			}
			...
			for (int i = 0; i < allNodeList.size(); i++) {
				...
				// 在节点中添加合并信息
				if (mergeNodeMap.containsKey(id)) { // 当前节点是否为合并节点,containsKey()判断是否包含指定的键名
					JsonObject mergeInfoObject = mergeNodeMap.get(id).getAsJsonObject();
					System.out.println(mergeInfoObject.toString());
					// 用于判断这个节点是否是一个删除节点
					String action = mergeInfoObject.get("action").toString();
					if (action.contains("delete")) { // 该节点在原来的基础上进行了删除
						JSONObject infoObject = new JSONObject();
						JSONArray propertiesArray = node.optJSONArray("properties");
						infoObject.put("@key", "info");
						infoObject.put("@value", "delete");
						propertiesArray.put(infoObject);
						
						// 更换图形的颜色，边框颜色变为红色
						System.out.println("更换图形的颜色，边框颜色变为红色");
						...
						new_foreground = foreground.substring(0, 11) + "5";
